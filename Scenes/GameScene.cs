...
public class GameScene : Scene
{
    ...
    public override void OnUpdate(double deltaTime, InputSnapshot snapshot, GameEngine engine)
    {
        ...
        if (kierunekRuchu.LengthSquared() > 0) _player.Velocity = Vector3.Normalize(kierunekRuchu) * _player.Speed;
        else _player.Velocity = Vector3.Zero;

        _player.Transform.Position += _player.Velocity * (float)deltaTime;

        // Dynamiczna wielkość chunka zależąca od jakości grafiki
            ...
            if (!_zaladowaneChunki.ContainsKey(coord))
            {
                ...
                for (int x = 0; x < chunkSize; x++)
                {
                    for (int z = 0; z < chunkSize; z++)
                    {
                        if (grid[x, z])
                        {
                            Wall w = new Wall((coord.x * chunkSize + x) * _cellSize, (coord.z * chunkSize + z) * _cellSize, _cellSize);
                            scianyChunku.Add(w); GameObjects.Add(w);
                        }
                    }
                }
        ...
        for (int i = 0; i < GameObjects.Count; i++)
        {
            if (GameObjects[i] is Wall wall)
            {
                ...
                {
                    float odleglosc = MathF.Sqrt(distSq);
